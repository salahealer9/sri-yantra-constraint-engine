"""
domain_sphere_v4_bsplit.py — EXPLORATORY copy of the v2 cone-edge driver with the
ONLY change being the SPLIT-DIMENSION choice: a deterministic boundary-aware split
PRIORITY (memo Path 3). It adds NO new criterion and does NOT straighten the
diagonal acos boundary (axis-aligned splits cannot); it only reorders already-sound
splits to reduce the signed seam intervals h+c-pi/2 and h+d-pi/2 faster (a faster
staircase). Same classify, pre-filter, Krawczyk, max_depth, r_cert, LIFO traversal.
Soundness-neutral: any split is sound, only efficiency changes; Gate-M unaffected.
Frozen v1.2 driver UNCHANGED.
...
def _seam_intervals(box):
    """Signed seam intervals S_c, S_d for the acos domain edges h+c=pi/2, h+d=pi/2.
    S straddles 0 <=> the box crosses that diagonal valid/invalid boundary."""
    (_b, (clo, chi), (dlo, dhi), _e, _g, (hlo, hhi)) = box
    Sc = (hlo + clo - HALF_PI, hhi + chi - HALF_PI)
    Sd = (hlo + dlo - HALF_PI, hhi + dhi - HALF_PI)
    return Sc, Sd

def _sw(S):
    """Seam-straddle width: interval width if it straddles 0, else 0 (resolved)."""
    lo, hi = S
    return (hi - lo) if (lo < 0.0 < hi) else 0.0

def _split_dim(box, w):
    """Boundary-aware split PRIORITY (deterministic). If neither seam straddles 0,
    use the v2 widest-axis split (unchanged). If one or both straddle, test splits
    along h, c, d and pick the dimension minimizing total post-split seam-straddle
    width; tie-break by larger coordinate width, then order h, c, d."""
    Sc, Sd = _seam_intervals(box)
    straddle = (Sc[0] < 0.0 < Sc[1]) or (Sd[0] < 0.0 < Sd[1])
    if not straddle:
        return int(np.argmax(w))                      # v2 behaviour, unchanged
    best_k = None; best_tot = None; best_w = None
    for k in (5, 1, 2):                               # h, c, d  (deterministic order)
        lo, hi = box[k]; mid = (lo + hi) / 2
        L = list(box); L[k] = (lo, mid)
        R = list(box); R[k] = (mid, hi)
        ScL, SdL = _seam_intervals(L); ScR, SdR = _seam_intervals(R)
        tot = _sw(ScL) + _sw(SdL) + _sw(ScR) + _sw(SdR)
        cw = w[k]
        if (best_tot is None
                or tot < best_tot - 1e-15
                or (abs(tot - best_tot) <= 1e-15 and cw > best_w + 1e-15)):
            best_tot, best_k, best_w = tot, k, cw
    return best_k
...
def enum(tlim=7200, max_depth=200, r_cert=3e-3, max_boxes=3_000_000, log_every=0):
    ...
    cert = []
    maxq = 1
    n_bsplit = [0]   # #times boundary-aware split fired (instrumentation)
    while stack and proc < max_boxes and time.time() - t0 < tlim:
        ...
        k = _split_dim(box, w)              # boundary-aware split priority (only change vs v2)
        if k in (1, 2, 5):
            _Sc, _Sd = _seam_intervals(box)
            if (_Sc[0] < 0.0 < _Sc[1]) or (_Sd[0] < 0.0 < _Sd[1]):
                n_bsplit[0] += 1
        lo, hi = box[k]; mid = (lo + hi) / 2
        L = list(box); L[k] = (lo, mid)
        R = list(box); R[k] = (mid, hi)
    ...
    return dict(boxes=proc, cert=cert, dom=dom, excl=excl, unres=unres,
                ...
                complete=(len(stack) == 0), queue_left=len(stack), n_bsplit=n_bsplit[0])
