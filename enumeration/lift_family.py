...
def emit_julia(path):
    ...
    A("")
    A("    # --- generic solve: ab-initio POLYHEDRAL (seed-free, complete by BKK) ---")
    A("    # Once the (Q+1)/x11a artifact loci are removed the generic member is")
    A("    # cheap to solve directly. Monodromy from a single seed under-saturated")
    A("    # (returned N*=1); polyhedral is complete by construction and measures N*.")
    A("    p0 = generic_params_at_seed(pools; rng=MersenneTwister(20260612))")
    A("    t_gen = @elapsed res0 = solve(F; target_parameters = p0)")
    A("    S = solutions(res0)")
    A("    Nstar = length(S)")
    A("    npaths = length(path_results(res0))")
    A("    println(\"N* ($fam) = $Nstar generic solutions  \",")
    A("            \"(polyhedral: $npaths paths, \", round(t_gen, digits=1), \"s)\")")
    A("")
    A("    # --- monodromy cross-check: seed with the polyhedral set, raised budget;")
    A("    #     a BKK-complete set must NOT grow under monodromy ---")
    A("    agree = true")
    A("    if Nstar > 0")
    A("        try")
    A("            mres = monodromy_solve(F, S, p0;")
    A("                max_loops_no_progress = 50, unique_points_rtol = 1e-10)")
    A("            agree = (length(solutions(mres)) <= Nstar)")
    A("            println(\"monodromy cross-check: \", length(solutions(mres)),")
    A("                    \" (<= N*: \", agree, \")\")")
    A("        catch err")
    A("            println(\"monodromy cross-check skipped: \", err)")
    A("        end")
    A("    end")
    A("")
    A("    # --- trace-test completeness certificate (hardened against empty set) ---")
    A("    cert_ok = false")
    A("    if Nstar == 0")
    A("        println(\"trace-test completeness: SKIPPED (empty generic set)\")")
    A("    else")
    A("        try")
    ...
    A("        \"generic_paths\" => npaths,")
    A("        \"monodromy_crosscheck_agree\" => agree,")
    A("        \"generic_solve_seconds\" => round(t_gen, digits=1)))")
    A("    write(results_io, '\\n'); flush(results_io)")
    A("")
    ...
    A("        # non-singular only: singular endpoints at non-generic 0/1 targets")
    A("        # are expected lifting artifacts, not isolated solutions.")
    A("        rs = real_solutions(res; only_nonsingular = true)")
    A("        adm = [s for s in rs if admissible(s)]")
    A("        ncert = -1")
    A("        try")
    A("            ncert = ndistinct_certified(certify(F,")
    A("                solutions(res; only_nonsingular = true);")
...
def emit_mv_probe(path, typ="Q3"):
    """Standalone polyhedral-solve probe: measures the ELIMINATED generic
    member's mixed volume (prints within ~30s) and its genuine isolated count.
    Seed-free, so it sidesteps the monodromy under-saturation entirely."""
    var_decl = " ".join(str(v) for v in all_vars)
    fixed = [jl(eq) for eq in list(chain_eqs.values()) + list(essential_eqs.values())]
    quad  = [jl(pool[k][0]) for k in QUAD]
    lin   = [jl(pool[k][0]) for k in LIN]
    pools = {"Q3":["quad","quad","quad"], "L1Q2":["lin","quad","quad"],
             "L2Q1":["lin","lin","quad"], "L3":["lin","lin","lin"]}[typ]
    L=[]; A=L.append
    A("# Mixed-volume probe — ELIMINATED 26-var generic member.")
    A("# Was 286,144 for the artifact-laden specific subset; this measures the")
    A("# eliminated generic member directly. The mixed volume prints within ~30s")
    A("# ('Computing mixed cells... mixed_volume: N'); let it finish for N* too.")
    A("# Polyhedral solve is seed-free -> no monodromy saturation issue.")
    A("using HomotopyContinuation, Random")
    A("Random.seed!(20260613)")
    A(f"@var {var_decl}")
    A(f"vars = [{', '.join(str(v) for v in all_vars)}]")
    A("fixed = [")
    for f in fixed: A(f"    {f},")
    A("]")
    A("quad = [")
    for q in quad: A(f"    {q},")
    A("]")
    ...
    for l in lin: A(f"    {l},")
    A("]")
    A("gslot(P) = sum(randn(ComplexF64)*p for p in P)")
    A(f"slots = [{', '.join('gslot('+p+')' for p in pools)}]   # type {typ}")
    A("F = System([fixed; slots]; variables = vars)")
    A(f"println(\"--- polyhedral solve of generic {typ} member (26-var eliminated) ---\")")
    A("result = solve(F)")
    A("println(\"mixed volume / paths tracked : \", length(path_results(result)))")
    A("println(\"total solutions              : \", nsolutions(result))")
    A("ns = solutions(result; only_nonsingular = true)")
    A("println(\"non-singular solutions (N*)  : \", length(ns))")
    A("println(\"real non-singular            : \", "
      "length(real_solutions(result; only_nonsingular=true)))")
    with open(path, 'w') as f:
        f.write("\n".join(L) + "\n")
    print(f"(5) mixed-volume probe written: {path}")

if __name__ == "__main__":
    validate()
    here = os.path.dirname(os.path.abspath(__file__))
    emit_julia(os.path.join(here, "family_solve.jl"))
    emit_mv_probe(os.path.join(here, "mv_probe.jl"), typ="Q3")
