"""
domain_sphere_v3_fifo.py — EXPLORATORY copy of the v2 cone-edge driver with the
ONLY change being traversal order: LIFO stack.pop() -> FIFO deque.popleft().
Same classify, same cone-edge pre-filter, same split dimension (argmax width),
same L-then-R child construction, same max_depth=200, same r_cert=3e-3. Single
variable under test: breadth-first vs depth-first. Adds a depth histogram (the
discriminator between a genuinely spread frontier and a merely-shallow run) and
peak-queue. Frozen v1.2 driver UNCHANGED.
...
def enum(tlim=7200, max_depth=200, r_cert=3e-3, max_boxes=3_000_000, log_every=0):
    """FIFO breadth-first. ONLY traversal order differs from the v2 LIFO driver:
    queue.popleft() instead of stack.pop(). Child construction (L then R, append
    right) is identical, so the single variable under test is pop order."""
    t0 = time.time()
    queue = deque([(list(B_SPHERE), 0)])     # FIFO frontier
    proc = dom = excl = unres = maxd = 0
    cert = []
    maxq = 1
    dhist = Counter()                        # depth -> #boxes processed at that depth
    while queue and proc < max_boxes and time.time() - t0 < tlim:
        maxq = max(maxq, len(queue))
        box, dep = queue.popleft()           # <-- FIFO (only change vs v2)
        proc += 1
        maxd = max(maxd, dep)
        dhist[dep] += 1
        if log_every and proc % log_every == 0:
            print(f"    [{proc} boxes  q={len(queue)}  peakq={maxq}  dom={dom} excl={excl} "
        ...
        queue.append((L, dep + 1)); queue.append((R, dep + 1))   # same L-then-R, append right
    return dict(boxes=proc, cert=cert, dom=dom, excl=excl, unres=unres,
                maxd=maxd, maxq=maxq, secs=round(time.time() - t0, 1),
                complete=(len(queue) == 0), queue_left=len(queue),
                depth_hist=dict(sorted(dhist.items())))
