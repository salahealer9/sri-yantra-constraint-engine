...
# ----- conservative elimination: Q7,Q8,Q9,Q12 and x11a (artifact sources;
#       see diagnose_artifacts.py).  26 -> 21 variables.  SymPy-derived. -----
_QU = [('Q7','U7'),('Q8','U8'),('Q9','U9'),('Q12','U12')]
...
for _q, _u in _QU:
    ...
    assert sp.simplify(_eqQ - (_D*_Q - _N)) == 0, f"{_q}: not D*Q-N"
    _S = -_eqU.coeff(_U, 0)
...
assert sp.simplify(_eqx - (_den*x11a - _num)) == 0
constraint_eqs['F1'] = sp.expand(_den*x11 - _num)
del chain_eqs['x11a']
_drop = {Q7, Q8, Q9, Q12, x11a}
aux_vars = [v for v in aux_vars if v not in _drop]
all_vars = orig_vars + aux_vars
F1_FOLD_FACTOR = 1 - U9 - g + U12            # F1_elim = factor * engine_F1
for _k in ('Q7+1', 'Q8+1', 'Q9+1', 'Q12+1'):
    denominators.pop(_k, None)
system = list(chain_eqs.values()) + list(constraint_eqs.values())   # rebuild (21)
...
def validate():
    ...
    checks = {
        'F1 == den*engine.F1'     : float(constraint_eqs['F1'].subs(subs)) - float(F1_FOLD_FACTOR.subs(subs))*Fe[1],
...
def emit_julia(path):
    ...
    ix1 = all_vars.index(x1) + 1     # 1-based Julia indices (shift after elimination)
    ix2 = all_vars.index(x2) + 1
    iw  = all_vars.index(w)  + 1
    lines = []
    ...
    lines.append("# PROBE: the headline is the 'mixed_volume:' line printed below")
    lines.append("# (computed in ~1 min, before path tracking). That number decides")
    lines.append("# whether per-subset ab-initio solving is feasible.")
    lines.append("result = solve(F)")
    lines.append("")
    lines.append("# certify ONLY non-singular solutions: interval certification")
    lines.append("# diverges on singular endpoints (rank-deficient Jacobian).")
    lines.append("ns   = solutions(result; only_nonsingular = true)")
    lines.append("cert = certify(F, ns)")
    lines.append("")
    lines.append("# admissible-branch filter (sign + range) over non-singular reals")
    lines.append("adm = filter(real_solutions(result; only_nonsingular = true)) do s")
    lines.append("    s[1]>1e-6 && s[2]>1e-6 && s[3]>1e-6 && s[4]>1e-6 && s[5]>1e-6 &&")
    lines.append(f"    s[{ix1}]>1e-9 && s[{ix2}]>1e-9 && s[{iw}]>1e-9 && s[2]<1 && s[3]<1")
    lines.append("end")
    lines.append("for s in adm; println(\"admissible: \", round.(s[1:5], digits=8)); end")
    lines.append("")
    lines.append("println(\"paths tracked: \", length(path_results(result)))")
    lines.append("println(\"non-singular solutions: \", length(ns))")
    lines.append("println(\"certified distinct (non-singular): \", ndistinct_certified(cert))")
    lines.append("println(\"admissible real (non-singular): \", length(adm))")
