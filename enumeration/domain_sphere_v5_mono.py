"""
domain_sphere_v5_mono.py — EXPLORATORY copy of the v2 cone-edge driver with the
ONLY change being the FN namespace it passes to the chain: MONO_AA_FN / MONO_DUAL_FN
(monotone across-inflection fallback for atan and acos; sin/cos/tan canonical). The
goal: remove INFLECTION-only SplitNeeded for atan/acos so those boxes get a (looser
but rigorous) enclosure instead of subdividing, while PRESERVING the acos domain-edge
split/exclude. cone_F / classify / certify_box / prefilter / split logic byte-identical
to v2; only the substrate forms for atan/acos change. Frozen v1.2 UNCHANGED.
LEVER-2 risk under test: the large-but-finite acos derivative near +-1 (up to ~1.9e2)
loosens the Krawczyk Jacobian; whether that hurts contraction is what Gate-M c4 and
the 400k head-to-head measure.
...
from chain_sphere import HALF_PI
from aar_sphere_v2_monotone import MONO_AA_FN as AA_FN, MONO_DUAL_FN as DUAL_FN  # ONLY change vs v2
