# Base-Coordinate Interval/Krawczyk Pilot — Pre-registration  (FROZEN v1.2)

**STATUS: FROZEN v1.2** (= frozen v1.0 + pre-pilot Amendment GM-1 to Gate-M
criterion 1, §5 + domain-semantics clarification, final section). Budget values are
locked (no placeholders), mirroring the plane precedent `domain_v3.py` and
deviating only where the spherical case forces it. Amendment GM-1 was made before
any exclusion/certification run, on the basis of a Gate-M finding that the
`cos(2x)/cos(x)` constraints F3/F4/F6 are intrinsically ill-conditioned near
x=π/2; it refines the fidelity metric without relaxing rigor (containment
unchanged) or altering the pilot outcome rules. The domain-semantics clarification
(final section) fixes the admissibility rule to full-Rao-chain-real, conservatively
(can only shrink the domain), again without changing budget, outcome rules, or
Gate-M criteria. SHA recorded at the freeze line.
...
**Freeze line (v1.2).** This pre-registration is frozen. v1.0 was frozen prior to
building the AA-trig extension and prior to any pilot run; v1.1 added Amendment
GM-1 (§5) before any exclusion/certification run; v1.2 adds the domain-semantics
clarification (final section) before the official pilot run. The SHA-256 of this
frozen file is recorded in the external provenance manifest (SHA256SUMS, git tag)
at commit — a file cannot contain its own hash, so the manifest is the
authoritative record, consistent with the project's byte-reproducibility practice.

Pilot pre-registration v1.2 — provenance: resolved by signed tag `pilot-pre-v1.2`

---

## Domain-semantics clarification (post-Gate-M, pre-official-pilot)

**The pilot certifies full Rao-chain-real admissibility; selected constraints are
evaluated on their dependency cone.**

Resolution of a semantic ambiguity surfaced during Gate-M review: a box is
*admissible* only if Rao's ENTIRE construction is real over it (all ~40 chain
nodes, including nodes not feeding the selected subset, e.g. radial acos
r16..r19). This is enforced by a full-chain domain guard in `domain_sphere.classify`
that runs before constraint evaluation: DomainError anywhere in the full chain →
`domain` (exclude); SplitNeeded anywhere → `split` (subdivide); clean → proceed.
Constraint *values* are still evaluated only on the dependency cone of the selected
constraints F1,F2,F3,F4,F6,F7 (`cone_F`), since unused constraints are not imposed.

Rationale (Gate-4 consistency): a subset root must correspond to a geometrically
valid spherical figure, not merely an algebraic zero of the selected equations. The
full-chain guard can only shrink/defer the admissible domain relative to cone-only,
never enlarge it, so it is the conservative and correct side. Soundness:
exclusion/certification occurs only when BOTH the full-chain guard returns `ok` AND
`cone_F` succeeds; constraint expressions (the iso `cos(2x)/cos(x)` terms) carry
their own transcendental domain conditions beyond the chain, and a seam in either
the chain or a constraint yields `split`. No box with any non-real node (chain or
constraint) is excluded or certified.

This clarification refines the domain rule of the frozen protocol; it does not
change the budget, the outcome rules, or the Gate-M criteria. `domain_sphere.py` is
patched accordingly (SHA recorded at re-freeze).

**Recorded future lever (not applied in v1.2).** The full-chain-real guard runs
the entire AA chain, so it may `split` not only on true domain boundaries but also
on inflection / tight-affine conditions inside *unused* nodes. This is sound but
more conservative than strictly necessary. If full-chain-real guarding later
becomes the bottleneck, a future lever could replace it with a domain-only guard
that checks only the validity inequalities (acos arguments in range, denominators
non-zero, poles avoided) for unused nodes, without requiring tight value
enclosures for them. For v1.2 the conservative full-chain guard is the deliberate,
correct choice.