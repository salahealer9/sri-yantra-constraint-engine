...
def emit_gate_x(path, subset=None, base=None, params=GATEX, wall_clock_s=None):
    ...
    L += ["]",
          ...
          "end",
          "",
          "# corroboration budget (separate, short): verify + trackback may not",
          "# exceed this; they are FORENSIC ONLY and never affect PASS/FAIL.",
          f"CORROB_CAP = 600.0",
          ...
          "elapsed1 = time() - t0",
          "",
          "# build a complete forensic snapshot from current state",
          "function build_dict(el)",
          "    Dict(",
          f"      \"subset\" => \"{list(gen['subset'])}\",",
          "      \"elapsed_s\" => round(el, digits=1),",
          "      \"n_generic\" => n_gen,",
          "      \"is_success\" => is_ok,",
          ...
          "end",
          "",
          "# PHASE 1 WRITE — primary verdict secured BEFORE any verification.",
          "write_forensic(build_dict(elapsed1))",
          'println("PHASE1 forensic written: is_success=", is_ok, " n_generic=", n_gen,',
          '        " elapsed=", round(elapsed1,digits=1), "s err=", err_kind)',
          "",
          "# PHASE 2 — OPTIONAL corroboration, separately time-boxed on its own thread.",
          "# Can neither strand the verdict nor block the archive (already written).",
          "if MR_ok",
          "    tcap = time()",
          "    vtask = Threads.@spawn begin",
          "        try; string(verify_solution_completeness(F, MR));",
          "        catch e2; \"exception:\"*string(typeof(e2)); end",
          "    end",
          "    while !istaskdone(vtask) && (time() - tcap) < CORROB_CAP; sleep(2.0); end",
          "    vsc = istaskdone(vtask) ? fetch(vtask) : \"timeout_\"*string(Int(CORROB_CAP))*\"s\"",
          "    # trackback only on small fibers and only within remaining corroboration budget",
          "    if n_gen > 0 && n_gen <= TB_MAX_FIBER && (time() - tcap) < CORROB_CAP",
          "        try",
          ...
          "            end",
          "            tb = \"ok_\"*string(length(cands))",
          "        catch e3; tb = \"exception:\"*string(typeof(e3)); end",
          "    elseif n_gen > TB_MAX_FIBER",
          "        tb = \"skipped_large_fiber_\"*string(n_gen)",
          "    end",
          "    # PHASE 2 WRITE — refresh forensic with corroboration (verdict unchanged).",
          "    write_forensic(build_dict(time() - t0))",
          "end",
...
def _selftest():
    ...
    chk("forensic written (build_dict + write_forensic)", "write_forensic" in txt)
    chk("6 sin-form constraints parametrized", txt.count(") - p") == 6)
    # phase-separation checks (the fix for the stranded-forensic failure)
    i_p1write = txt.find("PHASE 1 WRITE")
    i_verify = txt.find("verify_solution_completeness(F, MR)")   # the call, not the dict key
    chk("PHASE 1 forensic write occurs BEFORE verification in script order",
        0 < i_p1write < i_verify)
    chk("verification time-boxed (CORROB_CAP)", "CORROB_CAP" in txt)
    chk("verification on separate thread (Threads.@spawn)", "Threads.@spawn" in txt)
    chk("trackback gated by fiber size (TB_MAX_FIBER)", "TB_MAX_FIBER" in txt)
    chk("two forensic writes (phase1 + phase2 refresh)", txt.count("write_forensic(build_dict") == 2)
