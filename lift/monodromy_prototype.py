...
def emit(subset, base, path, seed=None):
    ...
    L = [f"# SPHERICAL monodromy prototype — subset {gen['subset']}",
         f"# system hash {LG.system_hash(gen)[:16]} ; seed = lifted base root",
         "using HomotopyContinuation, LinearAlgebra, Random",
         f"Random.seed!({seed if seed is not None else 0})",
         f"@var {decl}",
         f"@var {pdecl}",
         "eqs = ["]
    names = (["pyth_" + n for n in LG.ATOMIC_NAMES]
...
def _selftest():
    ...
    h = emit(SUB, NEWTON, "monodromy_1_2_3_4_6_7.jl", seed=1)
    txt = open("monodromy_1_2_3_4_6_7.jl").read()
