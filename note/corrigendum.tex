...
\keywords{Sri Yantra; Rao 1998; corrigendum; transcription errors; spherical geometry; plane geometry; constraint satisfaction; computational verification}
...
In transcribing the published equations exactly, however, we found that four of them, as printed, are not satisfied at Rao's own tabulated solutions. The discrepancies are localized and each resolves to a clear typographical or copy-template error. To the best of our knowledge, these specific transcription errors have not been previously identified or corrected in the literature. We report the corrections and the evidence for each.
...
The author used a large language model (Anthropic's Claude) as a computational and drafting aid during this work, including reimplementation of the construction, numerical checking, and manuscript preparation. It was not used for autonomous content creation. All mathematical results — the four reported corrections and the under-converged-row observation — were independently reproduced by the deposited code, which the author has reviewed. The author takes full responsibility for the content and conclusions of this Note.
...
The views expressed in this article are those of the author and INSA does not necessarily own responsibility for the same.
