# Correspondence

A dated log of external correspondence concerning this project (journal contact
regarding the Rao 1998 corrigendum, author contact, etc.), kept for provenance
and reproducibility.

Convention: one Markdown file per message, named `YYYY-MM-DD-<short-slug>.md`,
recorded verbatim as sent or received. Replies are added as their own dated
entries. Personal email addresses of third parties may be redacted in the public
record at the maintainer's discretion.

## Log

| Date | Direction | Counterparty | Subject | Entry |
|---|---|---|---|---|
| 2026-06-10 | sent | IJHS / INSA editorial office | Proposed corrigendum to Rao (1998) | `2026-06-10-ijhs-initial-contact.md` |
| 2026-06-11 | received | IJHS / INSA editorial office | Re: Proposed corrigendum (invitation to submit) | `2026-06-11-ijhs-reply.md` |
| 2026-06-11 | sent | IJHS Editorial Manager | Corrigendum submission | `2026-06-11-ijhs-submission.md` |
| 2026-06-11 | sent | Madhvendra (INSA) | Notification of submission (manuscript ID) | `2026-06-11-ijhs-notify-madhvendra.md` |
| 2026-06-12 | received | IJHS Editorial Office | Acknowledgment of corrigendum receipt | `2026-06-12-ijhs-acknowledgement.md` |
